class ScrapeCircuit
	...
	def self.get_circuits_list
		doc=Nokogiri::HTML(URI.open('https://www.f1-fansite.com/f1-circuits/'))
		table_circuit = doc.search('.motor-sport-results td').map{|circuit|circuit.text}.reject{|a|a.size < 2}
		table_url = doc.search('.motor-sport-results a').map{|circuit|circuit['href']}
		
		table_circuit.each_with_index do |d, i| 
			circuit = Circuit.create(name: d)
